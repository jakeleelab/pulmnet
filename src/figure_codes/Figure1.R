...
for (i in 1:nrow(loh)){
  for (j in c("ARID1A", "MEN1", "ATM", "NF1")){
    if (nrow(fg[startsWith(fg$sample, loh$study_id[i]),]) != 0 && !is.na(fg$lcn.em[startsWith(fg$sample, loh$study_id[i]) & fg$gene == j][1]) && fg$lcn.em[startsWith(fg$sample, loh$study_id[i]) & fg$gene == j][1] == "0"){
...
df <- ca[,c("study_id", "chromothripsis", "dic_EIF1AX", "dic_MEN1", "dic_ARID1A", "dic_ATM", "dic_KRAS", "dic_NF1", "dic_DN", "focalamp")]
for (i in 2:ncol(df)){
  df[,i] <- grepl("reference", df[,i])
}
df[-1] <- lapply(df[-1], function(x){ifelse(x == TRUE, 0, ifelse(x == FALSE, 1, x))})
rownames(df) <- df[,1]
df <- df[,colnames(df)[colnames(df) != "study_id"]]
mat <- t(as.matrix(df))
pma <- getPM(mat)
